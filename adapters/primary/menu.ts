  ...
  ui.createMenu("PA")
    ...
    .addSubMenu(
      ui
        .createMenu("e-Mails")
        .addItem(
          "Send reminder to those who did not verify the account",
          "menuItem3",
        )
        .addItem(
          "Send reminder to those who did not submit the peer assessment",
          "menuItem2",
        )
        .addItem("Send final peer assessment results", "announceItem"),
    )
    .addSubMenu(
      ui.createMenu("Links").addItem("Registration URL", "showRegItem"),
    )
    .addSubMenu(
      ui
        .createMenu("Install")
...
function getPAselected(pas: PeerAssessment[]): PeerAssessment | null {
  ...
  if (selection.getActiveSheet().getName() != PAS.sheet) {
    Browser.msgBox(
      "Please click on a row with a peer assessment in the " +
        PAS.sheet +
        " sheet.",
    );
    return null;
...
function installSheetsItem() {
  install();
  const logSheet = SpreadsheetApp.getActive().getSheetByName(LOG.sheet);
  if (logSheet == null) {
    Logger.log("installSheetsItem: Sheet not found: " + LOG.sheet);
    return;
  }
  logSheet.getRange(1, 1).setValue("INSTALLED");
...
function openPAitem() {
  ...
  if (pa.state != PaState.INACTIVE && pa.state != "") {
    Browser.msgBox(
      "The peer assessment " + pa.name + " has already been opened.",
    );
    return;
  }
...
function updateDeadlineMenuItem() {
  ...
  if (pa.deadline.getTime() < new Date().getTime()) {
    SpreadsheetApp.getActiveSpreadsheet().toast(
      "Deadline of " + pa.name + " is in the past! ",
    );
    return;
  }
  ...
  SpreadsheetApp.getActiveSpreadsheet().toast(
    "Deadline of " + pa.name + " changed to " + pa.deadline,
  );
}
...
function showRegItem() {
  ...
  if (registrationFormId != null)
    ...
    Browser.msgBox(
      "There is no registration URL. Click PA -> Install -> Install Registration & Verification form.",
    );
}
...
function showSidebar() {
  ...
  let html = template.evaluate().setTitle("Help").setWidth(500);
...
function onEdit(e: GoogleAppsScript.Events.SheetsOnEdit) {
  ...
  if (shName == PAS.sheet) {
    ...
    if (col == deadlineCol) {
      ...
      Browser.msgBox(
        "Run 'PA -> Peer Assessments -> Updates Deadlines' from the menu to update " +
          pa.name +
          "'s deadline to " +
          value,
      );
