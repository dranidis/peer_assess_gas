...
function createPeerAssessmentForm(
  ...
  form
    ...
    .setConfirmationMessage(
      "Check your email for successful submission of the peer assessment!",
    );
  ...
  for (let studentName of studentNames) {
    ...
    item
      .setTitle("Comments for " + studentName)
      .setHelpText(
        "Comments are welcome and required in cases of extreme assessment (i.e. 1 or 5)",
      );
  }
...
function installVerificationForm() {
  ...
  const form = FormApp.create("PA: Verification form")
    .setDestination(FormApp.DestinationType.SPREADSHEET, ss.getId())
    .setAllowResponseEdits(true)
    .setLimitOneResponsePerUser(false)
    .setConfirmationMessage(
      "Check your email to see if the registration is completed!",
    )
    /*
        allow users outside the domain to use the form
        */
  ...
  const emailVal = FormApp.createTextValidation()
    .requireTextIsEmail()
    // Ignore when working with clasp
    // @ts-ignore
    .build();
  ...
  const linksSheet = SpreadsheetApp.getActive().getSheetByName(LINKS.sheet);
  if (linksSheet == null) {
    sheetLog("Sheet not found: " + LINKS.sheet);
    return;
  }
