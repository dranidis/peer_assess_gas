...
function testPrintQuestions() {
  ...
  for (i = 0; i < act.length; i++) Logger.log("testGetQuestions: " + act[i]);
}

function testGetProjectkeyFromFormId() {
  Logger.log(
    getProjectkeyFromFormId("1z4zsCIVv7QC_3wg3TE97WQsOYwg_BHVse3Fo3SuIbFk"),
  );
}

function testGetPAresultsDebug() {
  var formId = "13as2R61syxjVnbDKEj_ZUokg50K5sgHqZKBB9BRpsUM";
...
function testGetPAresults() {
  var formId = "133oN6_obERXhBW6KyuxVcMypPA_a3aba5Ypq07eSnp4";
  var projectkey = "PROJ123";
  var settings = getSettings();
  var rawResponses = getFormResponses(formId, settings.domain);
  var responseMap = paScoreService.buildResponseMap(rawResponses);
...
function testReadArray() {
  var values = SpreadsheetApp.getActive()
    .getSheetByName("Peer Assessments")
    .getRange("C1")
    .getValue();
  Logger.log(values);
...
function testRenameResponseSheet() {
  var sh = getFormResponseSheet_(
    "1-P4TC2ku7WQmz-qn42oDB0p_YWkzU61aAiAehfBZPyk",
  );
...
function processTest() {
  ...
  for (var q = 0; q < questions.length; q++) {
    for (var s = 0; s < students.length; s++) {
      var string =
        questions[q] + " [" + students[s].fname + " " + students[s].lname + "]";
      Logger.log(string);
    }
  }
}
...
function testfunctionsavePeerAssessmentLinks() {
  savePeerAssessmentLinks(
    "pa1",
    "PROJ123",
    FormApp.openById("102713RRBqfUucvudWs682-0k2FC2puLBq_-AqvYYiIc"),
  );
}
function testGetVerURL() {
  Logger.log(
    (link = FormApp.openById(getVerificationFormId()).getPublishedUrl()),
  );
}
...
function testgetPaProjects() {
  ...
  for (var i = 0; i < pp.length; i++) Logger.log(pp[i]);
}
